...
var app = (function () {
    ...
    function create_fragment$2(ctx) {
    	...
    	let t1;
    	let label;
    	let t2_value = /*Products*/ ctx[0].list.length + "";
    	let t2;
    	let t3;
    	let button0;
    	let span0;
    	...
    	const block = {
    		...
    		p: function update(ctx, [dirty]) {
    			if ((!current || dirty & /*Products*/ 1) && t2_value !== (t2_value = /*Products*/ ctx[0].list.length + "")) set_data_dev(t2, t2_value);

    			if (dirty & /*Products, incrementItem, decrementItem, removeItem*/ 113) {
    				each_value = /*Products*/ ctx[0].list;
    				validate_each_argument(each_value);
    				group_outros();
    				validate_each_keys(ctx, each_value, get_each_context, get_key);
    ...
    function instance$2($$self, $$props, $$invalidate) {
    	...
    	const getProduct = (id, signal = null) => {
          ...
          fee
    			...
    			if (data && data.data && typeof data.data.getProduct === "object" && data.data.getProduct.id != null) {
    		...
    		add: product => {
    			Products.list.map((cartProduct, index) => {
    				...
    				if (product.id === cartProduct.id) {
    					...
    					$$invalidate(0, Products.__addFlag = true, Products);
    				}
    	...
    	Object.keys($$props).forEach(key => {
    		if (!~writable_props.indexOf(key) && key.slice(0, 2) !== '$$' && key !== 'slot') console_1.warn(`<CartContent> was created with unknown prop '${key}'`);
    	});
    	...
    	$$self.$$.update = () => {
    		if ($$self.$$.dirty & /*Products*/ 1) {
    			$$invalidate(1, subTotal = Products.list.reduce(
    				(previous, current) => {
